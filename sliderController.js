...
const {SliderOne} = require('../models/models')
...
class SliderController {
    async create(req, res, next) {
        try {
            ...
            if (!img) {
                ...
                const slider = await SliderOne.create({title, description, img: fileName})
    ...
    async getAll(req, res) {
        ...
        let slider;
        if (!limit) {slider = await SliderOne.findAndCountAll()}
        if (limit) {slider = await SliderOne.findAndCountAll({limit, offset})}
        return res.json(slider)
    }
    async destroy(req, res, next) {
        try {
            ...
            const slider = await SliderOne.destroy(
                {
                    where: {id},
                })
            return res.json(slider)
    ...
    async getOne(req, res, next) {
        try {
            const {id} = req.params
            const slider = await SliderOne.findOne(
                {
                    where: {id}
                })
            return res.json(slider)
    ...
    async update(req, res, next) {
        try {
            ...
            const slider = await SliderOne.update(
